...
export default function Works() {
  ...
  const ctaStyle = isMobileView
    ? { fontSize: s(20), lineHeight: sl(28), letterSpacing: 0 }
  ...
  return (
    <section ref={sectionRef} className="relative bg-white overflow-hidden" style={isMobileView ? { marginBottom: -1 } : {}}>
      ...
      <div className="relative z-10 flex flex-col items-center" style={contentStyle}>
          ...
          >
            {carouselSlides.map(({ src, offset, href }, i) => (
              <a
                ...
                style={{
                  width: isMobileView ? s(323) : fluid(266, 370),
                  aspectRatio: '500 / 400',
                  borderRadius: isMobileView ? s(26) : 30,
                  marginTop: `${offset}px`,
                }}
              >
                <img
        ...
        <div ref={ctaRef} className="flex flex-col items-center" style={isMobileView ? { gap: s(30), paddingLeft: isTablet ? s(40) : 16, paddingRight: isTablet ? s(40) : 16 } : { gap: 30, padding: '0 20px' }}>
          {isMobileView ? (
